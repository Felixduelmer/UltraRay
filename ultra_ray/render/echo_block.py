...
class EchoBlock(Object):
    """
        EchoBlock: stores the data of a specific region of the transducer
    ...
    # Reinitialize internal Tensor of RF block's data
    def clear(self):
        del self.m_data
    ...
    def develop(self, gamma=False, raw=False):
        ...
        combine_height = False
        if combine_height:
            # TODO: This can probably be made a lot more efficient / tidy
            B, C, T, D = dr.shape(res)
            D = 1
            # Initialize the flattened target array
            target_flat = dr.zeros(type(values), B * T * D)
            b_indices = dr.arange(UInt32, B)  # Batch indices
            t_indices = dr.arange(UInt32, T)  # Temporal indices
            d_indices = dr.arange(UInt32, D)  # Depth indices

            # Expand indices to match dimensions
            b_indices = dr.repeat(b_indices, C * T * D)        # Repeat B times across C, T, D
            t_indices = dr.tile(dr.repeat(t_indices, D), B * C) # Repeat T across D, and tile across B * C
            d_indices = dr.tile(d_indices, B * C * T)          # Tile D across B * C * T

            # Compute the flat indices using `fma`
            flat_indices = dr.fma(b_indices, T * D, dr.fma(t_indices, D, d_indices))

            # Perform scatter-reduce
            dr.scatter_reduce(dr.ReduceOp.Add, target_flat, values, flat_indices)
            
            output = TensorXf(target_flat, list([B, T]))
        else:
            output = TensorXf(values, list(res.shape[0:-1]))
